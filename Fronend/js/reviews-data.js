...
async function mnxGetVideosByCategory(slug) {
  ...
  return all.filter((v) => v.category === slug);
}
...
async function mnxLikeReview(placeId, reviewId) {
  ...
  await window.MNX_API.post(`/reviews/${encodeURIComponent(placeId)}/${encodeURIComponent(reviewId)}/like`);
  mnxInvalidateVideoFeed();
}

async function mnxUnlikeReview(placeId, reviewId) {
  if (!window.MNX_AUTH?.isLoggedIn()) throw new Error('กรุณาเข้าสู่ระบบก่อนกดหัวใจ');
  await window.MNX_API.delete(`/reviews/${encodeURIComponent(placeId)}/${encodeURIComponent(reviewId)}/like`);
  mnxInvalidateVideoFeed();
}
...
async function mnxGetComments(placeId, reviewId) {
  try {
    const data = await window.MNX_API.get(`/reviews/${encodeURIComponent(placeId)}/${encodeURIComponent(reviewId)}/comments`);
...
async function mnxAddComment(placeId, reviewId, text) {
  ...
  if (!text || !text.trim()) return { ok: false, reason: 'empty-text' };
  try {
    const data = await window.MNX_API.post(`/reviews/${encodeURIComponent(placeId)}/${encodeURIComponent(reviewId)}/comments`, { text: text.trim() });
    mnxInvalidateVideoFeed();
...
async function mnxDeleteComment(placeId, reviewId, commentId) {
  try {
    await window.MNX_API.delete(`/reviews/${encodeURIComponent(placeId)}/${encodeURIComponent(reviewId)}/comments/${encodeURIComponent(commentId)}`);
    mnxInvalidateVideoFeed();
    return { ok: true };
  } catch (err) {
    return { ok: false, message: err.message };
